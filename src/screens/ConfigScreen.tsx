...
import { Card, Button, Switch, Divider, ActivityIndicator } from 'react-native-paper';
...
const ConfigScreen: React.FC = () => {
    ...
    const handleManageSubscription = async () => {
        try {
            ...
            Logger.warn('Customer Center not available, opening store settings', error);
    ...
    const formatExpirationDate = (dateString?: string): string => {
        ...
        try {
            ...
            return date.toLocaleDateString(i18n.language === 'es' ? 'es-CL' : 'en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric',
            });
    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
                    ...
                    {loadingStatus ? (
                        ...
                        <View style={styles.subscriptionInfo}>
                            <View style={styles.statusRow}>
                                ...
                                <View style={[
                                    styles.statusBadge,
                                    { backgroundColor: subscriptionStatus?.isPro ? COLORS.gold : theme.colors.surfaceVariant }
                                ]}>
                                    <Text style={[
                                        styles.statusText,
                                        { color: subscriptionStatus?.isPro ? '#000' : theme.colors.onSurfaceVariant }
                                    ]}>
                            ...
                            {subscriptionStatus?.isPro && !subscriptionStatus.isLifetime && (
                                <>
                                    {subscriptionStatus.expirationDate && (
                                        <View style={styles.statusRow}>
                                            <Text style={[styles.statusLabel, { color: theme.colors.onSurfaceVariant }]}>
                                                {subscriptionStatus.willRenew ? t('config.renews') : t('config.expires')}:
                                            </Text>
            ...
            <Card style={[styles.card, styles.dangerCard, { backgroundColor: theme.colors.surface }]}>
                <Card.Content>
                    <View style={styles.sectionHeader}>
